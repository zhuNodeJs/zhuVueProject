...
<div class="goods">
  <div class="menu-wrapper" ref="menuWrapper">
    <ul>
      <li v-for="(item,index) in goods" :key='item.id' @click='menuClick(index,$event)' :class="index===menuCurrentIndex?'menu-item-selected':'menu-item'">
        <span class="text">
...
export default {
  ...
  methods: {
    menuClick (index, event) {
      if (!event._constructed) { // 自己默认派发事件时候(BScroll),_constructed被设置为true,但是浏览器原生并没有这个属性
        return
      }
      // 运用BScroll接口，滚动到响应的位置
    ...
    _initScroll () {
      ...
      this.foodsScroll = new BScroll(this.$refs.foodsWrapper, {
        click: true,
        probeType: 3 // 探针的作用，用来实时的检测滚动条的滚动
      })
      // 滚动监听事件
